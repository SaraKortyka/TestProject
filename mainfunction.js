...
static async loadBreadcrumb() {
  ...
  let path = "hauptreiter_" + parts[0]; // Start mit Hauptreiter
  const mainText = LayoutLoader.convertUmlauts(parts[0].charAt(0).toUpperCase() + parts[0].slice(1));
  html += ` › <a href="${path}.html">${mainText}</a>`;

  // Unterseiten iterativ hinzufügen
  for (let i = 1; i < parts.length; i++) {
    path += `_${parts[i]}`;
    const text = LayoutLoader.convertUmlauts(parts[i].charAt(0).toUpperCase() + parts[i].slice(1));
